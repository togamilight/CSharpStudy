...
类似数组初始化程序，任何实现了`IEnumerable`的类型，只有它为初始化列表中出现的每个元素都提供了一个恰当的公有的Add方法，就可以使用这个特性。Add方法可以接受多个参数，只要把值放在另一对大括号中
...
在大括号内所有表达式的编译时类型的集合中，如果所有类型都能隐式转换成其中一种类型，该类型即为数组类型。只有表达式的类型才会成为一个候选的数组类型
